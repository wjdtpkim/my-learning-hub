...
export const glossaryTerms: GlossaryTerm[] = [
  ...
  // === 뇌과학 ===
  { regex: /보상 예측 오류/g, tip: '뇌가 예상했던 보상과 실제 받은 보상이 다를 때 더 강하게 학습하는 원리. 게임의 예측 못한 보상 연출에 자주 쓰임' },

  // === AI 에이전트 동작 방식 ===
  { regex: /ReAct(\s*패턴)?/g, tip: '추론(Reasoning)과 행동(Acting)을 반복하며 AI가 스스로 판단해 다음 행동을 정하는 방식' },
  { regex: /시계열 데이터/g, tip: '시간 순서대로 쌓인 데이터. 심박수·수면 기록처럼 "언제 얼마였는지"가 중요한 데이터' },
  { regex: /무한 루프/g, tip: '어떤 동작이 끝나지 않고 계속 반복되는 상태. AI 에이전트가 판단을 잘못하면 같은 분석을 계속 반복하기도 함' },

  // === 노코드 · AI 기획 도구 ===
  { regex: /노코드/g, tip: '코드를 직접 짜지 않고, 말로 설명하거나 화면에서 클릭만으로 앱·사이트를 만드는 방식' },
  { regex: /\bNo-code\b/gi, tip: '코드를 직접 짜지 않고, 말로 설명하거나 화면에서 클릭만으로 앱·사이트를 만드는 방식' },
  { regex: /러버블|\bLovable\b/g, tip: '프롬프트로 설명하면 실제 작동하는 웹앱을 만들어주는 노코드 AI 도구' },
  { regex: /구글 AI Studio|\bAI Studio\b/g, tip: '구글의 AI 프롬프트 도구. 아이디어를 입력하면 기획안 같은 결과물을 정리해줌' },
];
